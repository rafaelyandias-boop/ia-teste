...
client.on("messageCreate", async (message) => {
  ...
  if (!userMessage) {
    return message.reply(
      "👋 Oi! Me mencione e faça uma pergunta que eu respondo! Exemplo: `@bot qual a capital do Brasil?`"
    );
  }
  ...
  try {
    ...
    const messages = [
      {
        role: "system",
        content:
          "Você é um assistente amigável e útil no Discord. Responda de forma natural, concisa e amigável. Use emojis ocasionalmente para deixar a conversa mais leve. Seja direto e objetivo nas respostas."
      }
    ];
    ...
    if (botResponse.length > 1900) {
      botResponse =
        botResponse.substring(0, 1900) +
        "... *(resposta muito longa, continuação cortada)*";
    }
